...
export default class QuestAns extends Component {
  constructor(props) {
    super(props)
    this.state = {
      photos: []
  ...
  render() {
    return (
      <div>
        {this.props.data && this.props.data.slice(0, 2).map((e, k) => (
          <div key={k} className="questionscontainer ">
            ...
            <div className="optionscontainer text-gray-600">
              <p>Helpful?<a href="#" className="underline ml-2 mr-1" onClick={() => e.question_helpfulness + 1}>Yes</a>({e.question_helpfulness})</p><p className=" ml-4">|</p><p><a href="#" onClick = {()=>this.props.addans()} className="underline ml-4">Add Answer</a></p>
            </div>
          </div>
        ))}
        <div className="questionscontainer  mt-3">
          <div className="w-2/3">
            ...
            <div className="photoscontainer">
              <div onClick={() => this.props.handle()} className="box-border mt-3 ml-6 h-12 w-20 p-4 border-2 border-gray-400 bg-gray-100  "></div>
              <div onClick={() => this.props.handle()} className="box-border mt-3 ml-3 h-12 w-20 p-4 border-2 border-gray-400 bg-gray-100  "></div>
              <div onClick={() => this.props.handle()} className="box-border mt-3 ml-3 h-12 w-20 p-4 border-2 border-gray-400 bg-gray-100  "></div>
            </div>
            <div>
              <div className="detailscontainer mt-2">
              ...
              <p onClick={() => this.props.load()} className="show font-sans font-bold ml-6 mt-4 text-gray-600 ">LOAD MORE ANSWERS</p>
            </div>
          </div>
          <div className="optionscontainer text-gray-600">
